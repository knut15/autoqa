...
139 개 TC 가 도트 하나씩으로 서고, 검증이 도는 동안 통과·실패가 실시간으로 켜진다.
왼쪽은 등록한 프로젝트들이다.

![TC 상세](./docs/images/case-detail.jpg)

실패한 TC 를 누르면 **왜 실패했는지**가 바로 뜬다 — 판정식, 절차, 기대 결과.
자동으로 판정되지 않은 것은 아래 버튼으로 직접 찍는다.
